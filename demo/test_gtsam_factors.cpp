...
int main() {
  ...
  // ---------- compare (g2o err = -gtsam err; reorder stereo to (uL,uR,v)) ----------
  ...
  std::cout << "MONO   g2o=[" << err_g2o_mono.transpose() << "]  gtsam=[" << err_gtsam_mono.transpose()
            << "]  residual=" << d_mono << "\n";
  std::cout << "STEREO g2o=[" << g2o_stereo_reord.transpose() << "]  gtsam=[" << err_gtsam_stereo.transpose()
            << "]  residual=" << d_stereo << "\n";

  bool ok = d_mono < 1e-6 && d_stereo < 1e-6;
  std::cout << (ok ? "PASS: GTSAM point factors match g2o edges\n" : "FAIL: mismatch\n");
  return ok ? 0 : 1;
}
