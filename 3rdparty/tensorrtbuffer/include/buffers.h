...
//!
    ...
    public:
        ...
        {
            // Create host and device buffers (TRT 10: IO-tensor API, no implicit batch)
        ...
        //!
        //! \brief Set the device address of every I/O tensor on the context (TRT 10 enqueueV3).
        //!
        void setTensorAddresses(nvinfer1::IExecutionContext* context) const
        {
            ...
            {
                const char* name = mEngine->getIOTensorName(i);
                context->setTensorAddress(name, mManagedBuffers[i]->deviceBuffer.data());
            }
        }
    ...
    private:
        void* getBuffer(const bool isHost, const std::string& tensorName) const
        {
            for (int i = 0; i < mEngine->getNbIOTensors(); i++)
                if (tensorName == mEngine->getIOTensorName(i))
                    return (isHost ? mManagedBuffers[i]->hostBuffer.data() : mManagedBuffers[i]->deviceBuffer.data());
            return nullptr;
        }
        ...
        void memcpyBuffers(const bool copyInput, const bool deviceToHost, const bool async, const cudaStream_t& stream = 0)
        {
            for (int i = 0; i < mEngine->getNbIOTensors(); i++)
            {
                ...
                const bool isInput
                        = mEngine->getTensorIOMode(mEngine->getIOTensorName(i)) == nvinfer1::TensorIOMode::kINPUT;
