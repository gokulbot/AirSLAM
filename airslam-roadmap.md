...
**✅ SESSION UPDATE 2026-07-09 — Stage 1 DONE (dynamic rejection is a POSE no-op); Stage 2–4 design refined.**
- **Dynamic rejection = 0 ATE** on office / cafe / VIODE-3_high (motion-gated YOLO; clean *same-map* A/B byte-identical: 0.0586/0.3362/5.0545 with hooks on == off). **AirSLAM's robust BA (Huber + 2-pass outlier reject) already subsumes moving points as outliers.** ⇒ **semantics-as-a-FILTER is redundant with robust estimation; the pose is already good; semantic value = map / objects / nav, NOT pose.** (Also confirmed: AnyLoc reloc 29.9% ≤ DBoW2 32.1% cross-session — Phase-1 holds.) Infra built + reusable: `YoloDetector` (TRT, `AIRSLAM_YOLO_CLASSES` env), motion-gating (reproj-residual > `AIRSLAM_MOTION_PX`) + `AIRSLAM_NO_DYNREJ` A/B flag, VIODE integration (`tools/viode/extract_viode_bag.py`, `configs/camera/viode.yaml`), Rerun logger (`tools/rerun/log_run.py`).
- **Stage 2 fork sharpened — the *textureless* case is the ONLY thing that justifies object-SLAM.** Textured objects → point-cluster (uses keypoints; but point-SLAM already had them). **Textureless → object-landmark:** track the DETECTION not keypoints (keypoint-assoc is *circular* for textureless) via IoU + **appearance/re-ID embedding** + 3D-pose prediction, OR **dense learned optical flow** (RAFT propagates through textureless; short-term only — drifts + breaks at occlusion → still need re-ID). Geometry from **multi-view bbox** and/or a **dense depth head** (mono ⇒ *scale-anchor to stereo per-KF*), not interior points.
- **Stage 3 object factor (measure-it, don't assume):** bbox-reprojection factor (**down-weighted** for shared pixels) + masked points used to **init / outlier-gate** the object, NOT as persistent `point↔object` edges. **The object node is the SPARSE factorization of "same rigid object"** — keeps the point-block block-diagonal (Schur stays cheap); fill-in only in the *reduced* system (object = fat landmark; marginalize points FIRST, keep object). **NO `point↔point` factors** (dense clique + redundant for static). Expect: better object map + cleaner association; **modest/measure** pose effect.
- **Stage 4 backend ≈ nvblox** (Voxblox-on-GPU, Isaac ROS): consumes AirSLAM **poses + depth + masks** → TSDF / ESDF / mesh + semantic voxel layer + dynamics-aware (occupancy-inconsistency + person-mask exclusion). Realizes "dense OUT of the estimator; dynamic rejection lives in the MAP, not the pose." The one seam we own: **metric depth in** (stereo direct, or learned-depth scale-anchored to stereo).
- **Semantic thread = dense flow+depth+seg multitask @ keyframe rate**, running *beside* the sparse-keypoint pose thread (the 2-thread design). Bonus: **flow − ego-motion = object 3D velocity** (nav prediction).
